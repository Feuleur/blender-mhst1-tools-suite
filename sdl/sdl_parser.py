    ...
    def read(self, recursive=False):
        ...
        if self.magic != 4998227:
            if self.path is not None:
                raise RuntimeError(str(self.path) + " is not a mhst1 sdl file (magic = " + str(self.magic) + ")")
            else:
                raise RuntimeError("Data is not a mhst1 sdl file (magic = " + str(self.magic) + ")")
...
if __name__ == "__main__":
    parser = SdlParser(path="v01d_m_00_a01.sdl")
    data = parser.read()
    import json
