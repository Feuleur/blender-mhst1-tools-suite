...
class ModParser():
    ...
    def read(self):
        ...
        #print(bbox_minx, bbox_miny, bbox_minz)
        ...
        #mesh_scaling = np.array([1,1,1])

        #print("vbuffer_size = ", vbuffer_size)
        #print(hex(group_offset))
        #print(hex(mesh_offset))
        #print(hex(vbuffer_offset))
        #print(hex(fbuffer_offset))
        #print(hex(_offset_4))
            ...
            for bone_i in range(4096):
                ...
                if remapping != 255:
                    ...
                    try:
                        ...
                        #armature_datas[bone_i]["remap"] = bone_i
                        #print("ERROR")
                        pass
            #for bone_i in range(400):
                #print(bone_i, remap_tmp[bone_i])
        ...
        for mesh_i in range(mesh_count):
            ...
            #mesh_info["unk_byte_1"] = self.bs.readUByte()
        ...
        mesh_datas = []
        #print(json.dumps(mesh_infos, indent=4))
        for mesh_i, mesh_info in enumerate(mesh_infos[:]):
            ...
            colors_np = None
            colors2_np = None
            #print(mesh_i, hex(mesh_info["encoding"]))
            #if mesh_i != 3:
                #continue
            #if mesh_i != 181:
                #continue
            ...
            if mesh_info["encoding"] == 0xd8297027:
                ...
                #tangent_np = (vert_bytes_int8[:,8:12]/127.0)
                ...
                vert_bytes_int8 = np.frombuffer(vert_bytes, dtype=np.int8).reshape([-1, int(mesh_info["blocksize"])])
                #print("GOT ONE 0x77d87021", mesh_i)
                ...
                #print("GOT ONE 0x667b1018", mesh_i)
                vert_bytes_float16 = np.frombuffer(vert_bytes, dtype=np.float16).reshape([-1, int(mesh_info["blocksize"]/2)])
                vert_bytes_int16 = np.frombuffer(vert_bytes, dtype=np.int16).reshape([-1, int(mesh_info["blocksize"]/2)])
                vert_bytes_uint8 = np.frombuffer(vert_bytes, dtype=np.uint8).reshape([-1, int(mesh_info["blocksize"])])
                vert_bytes_int8 = np.frombuffer(vert_bytes, dtype=np.int8).reshape([-1, int(mesh_info["blocksize"])])
                ...
                colors_np = (vert_bytes_int8[:,12:16])
                uv1s_np = (vert_bytes_float16[:,8:10])
                #print(np.argwhere(uv1s_np != uv1s_np))
                #print(uv1s_np)
                #print(hex(mesh_offset + mesh_info["blocksize"]*99))
                weights_values_np = np.ones(weights_names_np.shape, dtype=np.float32)
            ...
            #print(mesh_info["vert_count"], len(mesh_poss))

            #
            #print("face_sub", mesh_info["face_sub"])
            self.bs.seek(fbuffer_offset + mesh_info["face_sub"]*2)
            #print("face_loc = ", hex(self.bs.tell()))
            #print("LOCATION", hex(self.bs.tell()))
            #print(mesh_info["face_count"], mesh_info["face_count"]//3)
            #face_bytes = self.bs.readBytes(6 * (mesh_info["face_count"]//3))
            face_bytes = self.bs.readBytes((mesh_info["face_count"]*2))
            faces_np = None
            face_bytes_uint16 = np.frombuffer(face_bytes, dtype=np.uint16)
            #print(face_bytes_uint16)
            ...
            try:
                ...
                logger.warning("Got a submesh with a non-usable sliding window.")
                continue
            tmps_2 = []
            for tmp in tmps:
                tmp_2 = tmp.copy()
            ...
            #for x in range(faces_np):
                #for y in
            #print([y for y in [x for x in faces_np]])
            def nunique(a, axis):
                return (np.diff(np.sort(a,axis=axis),axis=axis)!=0).sum(axis=axis)+1

            faces_np = faces_np[nunique(faces_np, axis=1) == 3]
            #mesh_faces = np.vstack([np.lib.stride_tricks.sliding_window_view(np.delete(subsplit, subsplit==65535),3) for subsplit in splitted_faces]).tolist()
            #
...
if __name__ == "__main__":
    parser = ModParser(path="box120_108_00.mod")
    armature_datas, mesh_datas = parser.read()
    #print(mesh_datas[0]["faces"])
