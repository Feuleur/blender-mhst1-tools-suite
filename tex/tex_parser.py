...
class TexParser():
    def __init__(self, path=None, data=None):
        ...
        header_1 = self.bs.readUInt()
        header_2 = self.bs.readUInt()
        header_3 = self.bs.readUInt()
        
        
        version = header_1 >> 0 & 0xfff
        alpFlag = header_1 >> 28 & 0xf
        
        self.mipCount = header_2 >> 0 & 0x3f
        self.width = header_2 >> 6 & 0x1ffff
        self.height = header_2 >> 19 & 0x1ffff
    
        self.format = header_3 >> 8 & 0xff
        #print(path, self.format)
        self.mipOffsets = []
    ...
    def read(self, mipLevel=0):
        ...
        if self.format in DXGI_FORMAT.keys():
            ...
            elif DXGI_FORMAT[self.format] in ["BC5_TYPELESS", "BC5_UNORM", "BC5_SNORM", "BC5_UNORM_SRGB"]:
        ...
        if sys.platform == "win32":
            ...
            dll_close(self.lib._handle)
        return img_array, could_read

if __name__ == "__main__":
    import cv2
    parser = TexParser("./mo001_01_BM.tex")
    data = parser.read()
    #print()
