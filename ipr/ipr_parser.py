    ...
    def read(self):
        ...
        if self.magic != 7499881:
            if self.path is not None:
                raise RuntimeError(str(self.path) + " is not a mhst1 ipr file (magic = " + str(self.magic) + ")")
            else:
                raise RuntimeError("Data is not a mhst1 ipr file (magic = " + str(self.magic) + ")")

        _ = self.bs.readUInt()
        _ = self.bs.readFloat()
        object_info_counter = self.bs.readUInt()

        object_infos = []
        for object_info_i in range(object_info_counter):
            object_info = {}
            object_info["object_instance_count"] = self.bs.readUInt()
            _ = self.bs.readUInt()
            _ = self.bs.readUInt()
            _ = self.bs.readUInt()
            ...
            _ = self.bs.readUInt()
            _ = self.bs.readUInt()
            object_info["object_path_offset"] = self.bs.readUInt64()+16
            ...
            _ = self.bs.readUInt64()
            object_info["object_instance_offset"] = self.bs.readUInt64()+16
...
if __name__ == "__main__":
    parser = IprParser(path="./v01_01.ipr")
    data = parser.read()
    import json
    print(len(data))
