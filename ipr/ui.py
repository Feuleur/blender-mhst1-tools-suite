...
class MHST1_IMPORT_PT_IprSettingPanel_1(Panel):
    ...
    def poll(cls, context):
        ...
        return operator.bl_idname == "MHST1_IMPORT_OT_mhst1_ipr"
...
class MHST1_IMPORT_PT_IprSettingPanel_2(Panel):
    bl_space_type = 'FILE_BROWSER'
    bl_region_type = 'TOOL_PROPS'
    bl_label = "Texture Settings"
    bl_parent_id = "MHST1_IMPORT_PT_IprSettingPanel_1"
...
class MHST1_ImportIpr(bpy.types.Operator, ImportHelper):
    """Import from Ipr file format (.ipr)"""
    bl_idname = "mhst1_import.mhst1_ipr"
    bl_label = 'Import MHST1 Ipr'
    bl_options = {'UNDO'}
    filename_ext = ".ipr"
    ...
    def execute(self, context):
        candidate_modules = [mod for mod in addon_utils.modules() if mod.bl_info["name"] == "MH Stories 1 tool suite"]
        ...
        if addon_prefs.game_path == "":
            ...
            return {"CANCELLED"}
        #if addon_prefs.arc_extraction_path == "":
        game_path = addon_prefs.game_path
        #else:
            #game_path = addon_prefs.arc_extraction_path
        ...
        for filepath in filepaths:
            load_ipr(game_path, filepath, LOD=self.LOD, mesh_cache=mesh_cache, mesh_hashes=mesh_hashes, import_material=self.import_material, use_png_cache=self.use_png_cache, overwrite_png_cache=self.overwrite_png_cache)
