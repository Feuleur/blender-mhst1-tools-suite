...
class LmtParser():
    ...
    def read(self):
        ...
        if self.magic != 5524812 and self.version != 68:
        ...
        for animation_i, animation_info in enumerate(animations_infos):
            #if animation_info["animation_idx"] != 10:
                #continue
            ...
            for bone_info_i in range(animation_info["bone_info_count"]):
                ...
                track_info["default_values"] = [self.bs.readFloat() for _ in range(4)]
            ...
            for track_info in track_infos:
                ...
                if track_info["keyframe_array_offset"] != 0:
                    ...
                    bone_action_dict[track_info["bone_id"]][action_type] = {0:track_info["default_values"]}
                    if action_type == "rot":
                        if track_info["type"] < 3:
                            bone_action_dict[track_info["bone_id"]]["rot_referencial"] = "global"
                        else:
                            bone_action_dict[track_info["bone_id"]]["rot_referencial"] = "local"
                    if action_type == "pos":
...
if __name__ == "__main__":
    parser = LmtParser(path="em346_00.lmt")
    ##parser = LmtParser(path="em001_00.lmt")
    animation_data = parser.read()
    print(animation_data[0]["name"])
    import json
    print(animation_data[0]["bone_action"].keys())
    print(json.dumps(animation_data[0]["bone_action"][-1], indent=4))
